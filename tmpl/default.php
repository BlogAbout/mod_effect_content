...
$doc->addStyleSheet('modules/mod_effect_content/css/style.css?v0.3.7');
$doc->addStyleSheet('modules/mod_effect_content/css/effects.css?v0.3.7');

if ($params->get('pages_count') > 0) {
	$doc->addScript('modules/mod_effect_content/js/script.js?v0.3.7');
}
...
?>
	...
	<div id="module_effect_contents_<?php echo $module->id; ?>" class="list_contents <?php echo $params->get('moduleclass_sfx'); ?>">
		...
		foreach ($list as $id) {
				...
				if ($params->get('layout') == 'categories_joomla') {
					switch($params->get('joomla_image_from')) {
							...
							if (preg_match('@<img([^>]+)>@ui', $id->text, $images)){
								if (preg_match('@src="([^"]+)"@ui', $images[0], $images)){
									if (JFile::exists(JPATH_BASE . DS . $images[1])) {
										...
										if ($params->get('image_not_found') != '' && JFile::exists(JPATH_BASE . DS . $params->get('image_not_found'))) {
											$image = $params->get('image_not_found');
										} else {
											unset($image);
										}
									}
						...
						case '2': {
							...
							if ($image['image'] != '' && JFile::exists(JPATH_BASE . DS . $image['image'])) {
								$image = $image['image'];
							} else {
								if ($params->get('image_not_found') != '' && JFile::exists(JPATH_BASE . DS . $params->get('image_not_found'))) {
									$image = $params->get('image_not_found');
				...
				if ($params->get('layout') == 'articles_joomla' || $params->get('layout') == 'articles_joomla_id') {
					switch($params->get('joomla_image_from')) {
						...
						case '1': {
							...
							if ($image['image_intro'] != '' && JFile::exists(JPATH_BASE . DS . $image['image_intro'])) {
						...
						case '2': {
							$image = json_decode($id->images, true);
							if ($image['image_fulltext'] != '' && JFile::exists(JPATH_BASE . DS . $image['image_fulltext'])) {
			...
			// Готовим ссылку
			if ($params->get('layout') == 'categories_joomla') {
				$link = JRoute::_(ContentHelperRoute::getCategoryRoute($id->id));
			}
			if ($params->get('layout') == 'articles_joomla' || $params->get('layout') == 'articles_joomla_id') {
				$link = JRoute::_(ContentHelperRoute::getArticleRoute($id->id, $id->catid));
			}
			if ($params->get('layout') == 'categories_joomshopping') {
				$link = SEFLink('index.php?option=com_jshopping&controller=category&task=view&category_id=' . $id->id, 1);
			}
			if ($params->get('layout') == 'products_joomshopping' || $params->get('layout') == 'products_joomshopping_id') {
				$link = SEFLink('index.php?option=com_jshopping&controller=product&task=view&category_id=' . $id->category_id . '&product_id=' . $id->id, 1);
			}
			if ($params->get('layout') == 'categories_k2') {
			...
			/*if ($params->get('cols_count') > 0 && ($i == 0 || ($i % $params->get('cols_count') == 0))) {
				echo '<div class="row_cols">';
			}*/
			$i++;
		?>
				...
				<div class="mask">
					...
					<?php if ($params->get('display_read_more') == '1') { ?>
						<?php if ($params->get('link_all') == '0') { ?>
							<a href="<?php echo $link; ?>" class="readmore"><?php echo $params->get("read_more"); ?></a>
						<?php } else { ?>
		...
		<?php
			/*if ($params->get('cols_count') > 0 && ($i == count($list) || $i % $params->get('cols_count') == 0)) {
		...
		<?php if ($params->get('pages_count') > 0 && $pages > 0) { ?>
			<div class="p-paginations">
				<?php for ($j = 1; $j <= $pages; $j++) { ?>
					<span class="p-page<?php if ($j == 1) { echo ' current'; } ?>" data-page="page_<?php echo $j; ?>"><?php echo $j; ?></span>
				<?php } ?>
			</div>
			<div class="clear"></div>
		<?php } ?>
