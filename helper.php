...
class ModEffectContentHelper
{
    ...
    {
        if ($params->get("com_categories") && count($params->get("com_categories"))) {
    ...
    {
        if ($params->get("articles_id") && count($params->get("articles_id"))) {
            $db = \JFactory::getDBO();
            $query = $db->getQuery(true);
    ...
    {
        if ($params->get("jshoppingcategories") && count($params->get("jshoppingcategories"))) {
            require_once(JPATH_SITE . '/components/com_jshopping/bootstrap.php');

            $lang = \JSFactory::getLang();
            $field_name = $lang->get('name');
            $field_description = $lang->get('description');

            $id_categories = implode(',', $params->get("jshoppingcategories"));

            $db = \JFactory::getDBO();
            $query = $db->getQuery(true);
            $query->select('`category_id` AS id, `' . $field_name . '` AS title, `' . $field_description . '` AS text, `category_image` AS image')
            ...
            switch ($params->get("field_sort")) {
                ...
                case '2':
                {
                    $query->order($db->quoteName($field_name) . ' ' . $params->get("type_sort"));
                    break;
                }
                case '3':
                {
    ...
    public static function getListProductsJoomshopping(&$params)
    {
        if ($params->get("jshoppingcategories") && count($params->get("jshoppingcategories"))) {
            ...
            $db = \JFactory::getDBO();
            $query = $db->getQuery(true);
            $query->select('prod.`product_id` AS id, prod.`' . $field_name . '` AS title, prod.`' . $field_description . '` AS text, prod.`image`, prod.`product_price`, prod.`product_date_added` as date_publish, curr.`currency_code`, cat.`category_id`')
            ...
            switch ($params->get("field_sort")) {
                ...
                case '2':
                {
                    $query->order('prod.' . $db->quoteName($field_name) . ' ' . $params->get("type_sort"));
                    break;
                }
                case '3':
                {
    ...
    {
        if ($params->get("products_id") && count($params->get("products_id"))) {
            require_once(JPATH_SITE . '/components/com_jshopping/bootstrap.php');
    ...
    {
        if ($params->get("k2categories") && count($params->get("k2categories"))) {
    ...
    {
        if ($params->get("k2items_id") && count($params->get("k2items_id"))) {
            $db = \JFactory::getDBO();
            $query = $db->getQuery(true);
    ...
    {
        if ($params->get("adsmanagercategories") && count($params->get("adsmanagercategories"))) {
    ...
    {
        if ($params->get("ads_id") && count($params->get("ads_id"))) {
            $db = \JFactory::getDBO();
            $query = $db->getQuery(true);
